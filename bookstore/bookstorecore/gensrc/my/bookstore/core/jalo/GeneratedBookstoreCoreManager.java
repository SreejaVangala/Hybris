...
import my.bookstore.core.jalo.CustomWcmsComponent;
import my.bookstore.core.jalo.ElectronicsColorVariantProduct;
import my.bookstore.core.jalo.FirstCronJob;
...
public abstract class GeneratedBookstoreCoreManager extends Extension
{
	...
	**/
	...
	public CustomWcmsComponent createCustomWcmsComponent(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( BookstoreCoreConstants.TC.CUSTOMWCMSCOMPONENT );
	...
	public FirstCronJob createFirstCronJob(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( BookstoreCoreConstants.TC.FIRSTCRONJOB );
			return (FirstCronJob)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating FirstCronJob : "+e.getMessage(), 0 );
		}
	}
	
	public FirstCronJob createFirstCronJob(final Map attributeValues)
	{
		return createFirstCronJob( getSession().getSessionContext(), attributeValues );
	}
	...
	{
		return (String)item.getProperty( ctx, BookstoreCoreConstants.Attributes.Product.RATING);
	}
	...
	{
		item.setProperty(ctx, BookstoreCoreConstants.Attributes.Product.RATING,value);
	}
	...
	/**
	 * <i>Generated method</i> - Getter of the <code>Product.review</code> attribute.
	 * @return the review
	 */
	public String getReview(final SessionContext ctx, final Product item)
	{
		return (String)item.getProperty( ctx, BookstoreCoreConstants.Attributes.Product.REVIEW);
	}
	 ...
	 * @return the review
	 */
	public String getReview(final Product item)
	{
		return getReview( getSession().getSessionContext(), item );
	}
	
	/**
	 * <i>Generated method</i> - Setter of the <code>Product.review</code> attribute. 
	 * @param value the review
	 */
	public void setReview(final SessionContext ctx, final Product item, final String value)
	{
		item.setProperty(ctx, BookstoreCoreConstants.Attributes.Product.REVIEW,value);
	}
	 ...
	 * @param value the review
	 */
	public void setReview(final Product item, final String value)
	{
		setReview( getSession().getSessionContext(), item, value );
	}
